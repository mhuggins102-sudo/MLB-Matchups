  ...
  if (missing.length) {
    ...
    // Make it obvious what’s wrong (works even if showAlert isn't available yet)
    alert('Site scripts failed to load: ' + missing.join(', ') +
          '\n\nLikely a blocked CDN request. Open DevTools → Network and look for failed requests to cdn.jsdelivr.net / cdn.tailwindcss.com.');

    // IMPORTANT: stop executing the rest of the script so we don’t throw again
...
window.addEventListener('unhandledrejection', (e) => {
  ...
  try {
    ...
    // Strip ?c= so refreshes don’t keep trying a broken challenge
...
function submitGuess() {
    ...
    if (playerCorrect) {
        pts += 200;
        resultEmoji = '🟧'; // CHANGED: Orange for Range Miss
        if (rangeCorrect) {
            resultEmoji = '🟩';
            const curCat = getCurrentCat(catKey);
    ...
    if (!useAnimations) {
        ...
        if (playerCorrect) {
            ...
            if (rangeCorrect) {
                const rangePts = roundTotal - 200 - (resultEmoji === '🎯' ? 100 : 0);
...
function toggleFormat() {
    ...
    if (!dom.uiPickEm.classList.contains('hidden')) {
        ...
        if (threshEl && catKey) {
            ...
            const op = catConfig.lowerBetter ? "≤" : "≥";
...
async function shareGame() {
        ...
        try {
            let baseDomain = "https://mlb-matchups.mhuggins102.workers.dev"; 
            ...
            // COMBINED SUBTITLE
            const subtitleText = `${modeText} • ${poolText} • ${eraText}`;
...
function startRoundSideBySide() {
    ...
    function submitSideBySide() {
        ...
        setTimeout(() => {
            ...
            else if(correctCount === 8) sbsEmoji = '🟧'; // Orange
